...
fn execute_get(key: String) -> Result<Option<String>> {
  let conn = get_proxy().lock().unwrap().get_conn().unwrap();

  match RocksDataStore::get(key.as_str(), conn) {
...
fn execte_put(key: String, val: Option<String>) -> Result<()> {
  ...
  let res =  match RocksDataStore::put(key.as_str(), val.unwrap().as_str(), conn) {
    Ok(()) => Ok(()),
...
fn execute_delete(key: String) -> Result<()> {
  ...
  match RocksDataStore::delete(key.as_str(), conn) {
    Ok(()) => Ok(()),
