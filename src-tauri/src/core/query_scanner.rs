...
impl<'a> QueryScanner<'a> {
    ...
    fn next_oracle_result(
        &self,
        _params_iter: &RefCell<ParameterIterator<Box<dyn oracle_ToSql>, String>>,
    ...
    fn next_postgres_result(
        ...
        let _prepared_stmt = param_iter_ref.prepared_stmt();
        todo!()
    ...
    pub fn map_oracle_param_seeds(_schema: String, _query: &Query) -> Result<ParamSeeds> {
        todo!()
    ...
    pub fn map_postgres_param_seeds(_schema: String, _query: &Query) -> Result<ParamSeeds> {
        todo!()
        // let emptry_params = vec![];
        // crate::proxies::postgres::get_runtime()
        //     .lock()
