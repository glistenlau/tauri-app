...
impl<'a> SQLClient for PostgresProxy<'_> {
  #[tokio::main]
  async fn execute<'b>(&self, statement: &str, parameters: &[&str]) -> SQLReponse {
    let now = Instant::now();
    let stmt;
    if parameters.len() > 0 {
      ...
      while let (Some(part), Some(param)) = (stmt_iter.next(), param_iter.next()) {
        println!("filled_stmt: {}", filled_stmt);
        filled_stmt.push_str(param);
        filled_stmt.push_str(part);
      }
      if (stmt_iter.next(), param_iter.next()) != (None, None) {
